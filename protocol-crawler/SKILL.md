...
> 首次可写"全称（缩写）"，后续只用缩写。压缩上下文时加：`[TERMINOLOGY: See protocol-crawler skill terminology table]`

`pc`=协议爬虫 · `yzm`=验证码 · `fk`=风控 · `nx`=逆向 · `qm`=签名 · `zw`=指纹 · `hx`=混淆 · `hy`=还原 · `dl`=代理 · `zh`=账号 · `pj`=破解 · `rg`=绕过 · `zrgz`=注入钩子 · `2cc`=2Captcha · `cs`=CapSolver · `cf`=Cloudflare
...
**检查**：检测 Superpowers 是否已安装——检查 skills 目录下是否存在 `superpowers` 相关目录（如 `~/.codex/skills/` 下的 superpowers 子目录）。若无法确认，尝试引用 Superpowers 提供的任一 skill 名称（如 `systematic-debugging`）看是否被识别。
...
2. **执行需求对齐**：
   - **若已安装 `product-requirements` skill**：使用该 skill 进行交互式需求收集并生成 `docs/PRD.md`（该 PRD 将被步骤 2 方案对齐、步骤 3 接口分析、步骤 5 实现直接引用）
...
**DevTools 连接排错**（连接失败时按顺序检查）：

| 症状 | 可能原因 | 处理 |
| --- | --- | --- |
| 连接被拒 | Chrome 未以调试模式启动 | 检查是否通过 `Google Chrome 9222` 快捷方式启动 |
| 页面列表为空 | 未打开目标页面 | 在调试浏览器中手动打开目标站点 |
| 操作超时 | 页面未加载完成 | 先 `wait_for` 关键元素再操作 |
| 找不到网络请求 | 打开页面前未开始监听 | 先 `list_network_requests` 确认监听已启动，再触发操作 |
...
> 📢 **进度汇报点**（多步实现时）：核心请求构建完成后、冒烟测试前，告知用户"请求构建已完成，准备冒烟测试"。

**HTTP 客户端**：强制 `curl_cffi`（代码见强制规则 §1），详见 `references/humanization/network-layer.md`。
...
> ⛔ **冒烟测试未通过则禁止进入步骤 6（CI 门禁）。**

> 📢 **进度汇报点**：冒烟测试通过后，告知用户"核心功能已验证，开始 CI 门禁检查"。
...
- 编码采用 `test-driven-development` skill 的 RED-GREEN-REFACTOR 流程：
  - RED：先写 `test_fetch_page_200()`、`test_handle_429()` 等测试
  - GREEN：实现 `_fetch_page()` 使测试通过
  - REFACTOR：提取公共逻辑到 `client.py`
...
| `humanization/proxy-management.md`                       | dl 类型选择、存活校验、轮换策略、失败淘汰                          | 需要 dl 轮换 / 被 IP 封禁时    |
